...
//
// gbest update path — pick one and commit:
//   Single-thread device kernel "kernel_commit_gbest" that reads
...
//       + record gbest_history[it] from inside that same kernel.
//
//  gbest history logging — append d_gbest_history[it] = current
