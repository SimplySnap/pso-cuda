#!/usr/bin/env python3
"""Shape C2 for Table A (CUDA API breakdown): per-component lines vs N.

Two panels side-by-side (D=30 left, D=300 right) so we don't need to
encode D as a line style. Inside each panel: one line per API component,
ms (log-y) vs per-rank N (log-x). Fixed at np=4.

Reads bench/nsight_matrix.csv, writes bench/fig_nsight_api_lines.png.
"""
...
OUT = os.path.join(HERE, "fig_nsight_api_lines.png")
...
COMPONENTS = [
    # name, color, marker
    ("cudaDeviceSync",       "#8172B2", "o"),
    ("cudaMemcpy",           "#C44E52", "s"),
    ("cudaLaunchKernel",     "#DD8452", "^"),
    ("cudaMemcpyFromSymbol", "#888888", "D"),
]
...
def canonicalize(name):
    for needle, canon in API_MAP:
        if needle in name:
            return canon
    return None  # exclude "other" from the line chart for clarity
...
for ax, D in zip(axes, DS):
    ...
    # Mark OOM region for D=300 N=8M
    if D == 300:
...
axes[0].set_ylabel("CUDA API time (ms, log scale)",
...
axes[1].legend(fontsize=STYLE["legend_size"] + 1,
               ...
               frameon=False, title="API call", title_fontsize=10)

fig.suptitle(
    "Table A — CUDA API time vs N at np=4 (Nsight, rank 0, ring)",
