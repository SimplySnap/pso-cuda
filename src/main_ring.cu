...
int main(int argc, char** argv) {
    /*
    MPI ring-topology island PSO entry point.
    Each rank owns one island on one GPU, syncing via ring migration every
    sync_interval iterations.
    ...
    Structure:
        ...
        - build PSOConfig with on_sync = island_migrate_ring
        - pso_run()
